...
public class MillstoneCategory implements DisplayCategory<MillstoneDisplay> {
    public static final Identifier TEXTURE_ID = Peony.id("textures/gui/millstone/millstone_gui.png");
    ...
    public Text getTitle() {
        return Text.translatable(PeonyTranslationKeys.REI_CATEGORY_MILLING);
    }
    ...
    public List<Widget> setupDisplay(MillstoneDisplay display, Rectangle bounds) {
        ...
        // millstone photo
        widgets.add(Widgets.createTexturedWidget(TEXTURE_ID, startPoint.x + 48,  startPoint.y + 46, 0, 0, 30, 20, 64, 64));
        // rotation times tooltip
        widgets.add(Widgets.createTexturedWidget(TEXTURE_ID, startPoint.x + 85, startPoint.y + 34, 22, 20, 9, 9, 64, 64));
        // rotation times tooltip
        widgets.add(Widgets.createDrawableWidget((context, mouseX, mouseY, delta) -> {
            TextRenderer renderer = MinecraftClient.getInstance().textRenderer;
            context.drawText(renderer,
                    Text.translatable(PeonyTranslationKeys.REI_MILLING_TIMES, display.getMillingTimes()),
                    startPoint.x + 100, startPoint.y + 35, 1325400064, false);
        }));
        // arrow
        widgets.add(Widgets.createTexturedWidget(TEXTURE_ID, startPoint.x + 84, startPoint.y + 48, 0, 20, 22, 15, 64, 64));
        // input slot
